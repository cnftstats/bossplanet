...
if(nrow(.CNFTS) != 0) {
  ...
  CNFTS <- CNFTS[order(-sold_at), .(asset, price, district, 
...
JPG[, district       := get_asset_data(asset_id)$onchain_metadata$District, 1:nrow(JPG)]
...
JPGS[, district       := get_asset_data(asset_id)$onchain_metadata$District, 1:nrow(JPGS)]
JPGS[, xcoord         := extract_num(strsplit(asset, ",")[[1]][1]), 1:nrow(JPGS)]
JPGS[, ycoord         := extract_num(strsplit(asset, ",")[[1]][2]), 1:nrow(JPGS)]

JPGS <- JPGS[order(-sold_at), .(asset, price, district,
                                sold_at, sold_at_hours, sold_at_days, market, xcoord, ycoord)]
...
# Database evolution -------------------------------------------------------------------------------
DTE <- copy(DT)
if (file.exists("data/DTE.rds")) {
  cat("File data/DTE exists:", file.exists("data/DTE.rds"), "\n")
  DTE_old <- readRDS("data/DTE.rds")
  DTE <- rbindlist(list(DTE, DTE_old))
  DTE <- DTE[difftime(time_now, data_date, units = "hours") <= 24] # Only retain last 24 hours
}
saveRDS(DTE, file = "data/DTE.rds")

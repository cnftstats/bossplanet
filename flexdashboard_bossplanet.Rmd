...
### Minted
```{r fig.height=10}
CAP <- RAR[, .(minted = .N), district]
CAP <- CAP[, sprintf("District %s = %d lands", district, minted)]
CAP <- paste0(CAP, collapse = "\n")
CAP <- paste0("Total = ", nrow(RAR), " lands\n", CAP)

gg <- ggplot(RAR, aes(x = xcoord, y = ycoord)) +
  geom_point(color = "grey60", size = .3) +
  scale_x_continuous(breaks = c(-180, -60, +60, +180)) +
  scale_y_continuous(breaks = c(-180, -60, +60, +180)) +
  labs(x = "", y = "", title = "Boos Planet minted lands", subtitle = CAP) +
  theme(panel.grid.minor = element_blank(),
        panel.grid.major = element_line(colour = "grey80"),
        panel.background = element_rect(fill = 'transparent'),

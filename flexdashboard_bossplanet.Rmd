...
X[price %between% c(0, 2499), price_range := trunc(price/300)*300]
X[price >= 2500, price_range := 2500]
...
X[price_range %between% c(0, 2499),  x_label := paste0(price_range, "-", price_range + (300-1))]
X[price_range >= 2500,               x_label := "2500+"]
...
X <- X[complete.cases(price_range), .(price_range, price, market, district_txt)]
CAP <- X[, .(floor = min(price), listings = .N), district_txt][order(district_txt)]
.caption <- paste0(CAP[, sprintf("%s: Sale floor = %s | Sales = %d", district_txt, floor, listings)],
                   collapse = "\n")
X <- X[, .N, .(price_range, market, district_txt)][order(price_range)]
X[price_range %between% c(0, 2499), x_label := paste0(price_range, "-", price_range + (300-1))]
X[price_range >= 2500,              x_label := "2500+"]
...
gg <- ggplot(DT, aes(x = xcoord, y = ycoord, color = price_rank, customdata = link,
                     text = paste0(asset,
                                   "</br></br>", paste("Price:", price,
                                                       ...
                                                       "</br>Market:", market)))) + 
  geom_point(size = .3) +
  scale_color_gradient(low = 'steelblue3', high = 'firebrick3', 
...
> Click on the data points to open the listings. Click and drag your mouse to zoom.
